...
"""''';
  ...
  Future<List<AiCutSegment>> splitVideoIntoSegments({
    ...
    final systemPrompt = isCliffhangerMode
        ? '''
Ты — гениальный TikTok/Reels продюсер и сценарист, мастер создания вирусных сериалов с удержанием 100%.
Твоя задача — разделить транскрипт видео на серии, обрывая каждую часть СТРОГО НА САМОМ ИНТРИГУЮЩЕМ МЕСТЕ (КЛИФФХЭНГЕР).

ПРАВИЛА ВИРУСНОГО КЛИФФХЭНГЕРА:
1. Каждая серия ОБЯЗАНА обрываться перед раскрытием тайны, на шокирующем вопросе, неожиданном повороте сюжета или напряженной мысли, чтобы зрителю было НЕВОЗМОЖНО не посмотреть следующую часть!
2. Хронометраж свободный: от ~25 до ~85 секунд (главное — эмоциональный пик и незавершенный ответ).
3. Каждая серия объединяет диапазон номеров фраз от start_phrase до end_phrase (номера с решеткой #).
4. Серии идут строго подряд без пропусков (серия 2 начинается со следующей фразы после серии 1).
5. Придумай интригующий вирусный заголовок (hook) капсом из 3-5 слов без кавычек и точек.

ВЕРНИ ТОЛЬКО ЧИСТЫЙ СТРОГИЙ JSON-МАССИВ:
[
  {
    "part_number": 1,
    "start_phrase": 1,
    "end_phrase": 6,
    "hook": "ЧТО ОНИ НАШЛИ ВНУТРИ",
    "summary": "Обрыв перед вскрытием двери"
  },
  {
    "part_number": 2,
    "start_phrase": 7,
    "end_phrase": 14,
    "hook": "ТАЙНА БЫЛА РАСКРЫТА",
    "summary": "Кульминация и новый вопрос"
  }
]'''
        : '''
...
[
  ...
  },
  {
    ...
    final modeDesc = isCliffhangerMode
        ? 'РЕЖИМ: АВТО-КЛИФФХЭНГЕР (режь строго на пиках интриги и неразрешенных вопросах, хронометраж свободный 25-85с).'
